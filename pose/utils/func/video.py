...
def determine_torso_and_body_range(
    keypoints, target_keypoints, center_y, center_x, KEYPOINT_DICT
):
...
def to_gif(images, path="./animation.gif", fps=30):
    """Converts image sequence (4D numpy array) to gif."""
    imageio.mimsave(path, images, duration=(1000 * 1 / fps))
    print(f"Saving video to {path}")


def run_inference_with_crop_region(model, image, crop_region, crop_size):
    """Runs model inferece on the cropped region.
